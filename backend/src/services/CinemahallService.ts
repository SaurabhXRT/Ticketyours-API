    ...
    try {
      const cityCinemaHalls = await CityCinemaHall.findAll({
        ...
        include: [{
          model: CinemaHall,
          as: "cinemaHall",
          attributes: ['id', 'name', 'location', 'zipcode'], 
        }],
      });
      const cinemaHalls = cityCinemaHalls.map((cityCinemaHall) => cityCinemaHall.cinemaHall.toJSON());
      return cinemaHalls;
    } catch (error) {
      console.error(`Error fetching cinema halls for city ID ${cityId}:`, error);
    ...
    try {
      const cityChecks = await CityCheck.findAll({
        ...
        include: [{
          model: CinemaHall,
          as: 'cinemaHall',
          attributes: ['name', 'location', 'zipcode'],
        }],
      });
      ...
      console.error(`Error fetching cinema halls for city ID ${cityId} and movie ID ${movieId}:`, error);
      throw new Error(`Failed to fetch cinema halls for city ID ${cityId} and movie ID ${movieId}`);
    }
  }
}
